...
`text` prefixes such as `text-text-content-primary` but we feel that this is necessary for our
...
Add a new token to the `text` block (ie, `content-tertiary`) with the color that `surface.content`
was.
...
```tsx
{
  text: {
    'content-primary': colorPalette['athens-gray'],
    'content-secondary': colorPalette.gray[600],
+   'content-tertiary': colorPalette['waterloo']
  }
}
```
