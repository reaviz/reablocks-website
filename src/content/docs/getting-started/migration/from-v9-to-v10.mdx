...
In 10.x version we've introduced new [Unify Design System](https://unifydesignsystem.com/#process) with component styles update and several breaking changes related to the theme.

If you don't want to use updated styles you can continue using the previous version of Reablocks theme:

```tsx
import { themeV9 } from 'reablocks';
```
...
and just merge it with your custom theme. Pay attention that theme interface for some component was changed, so you need to update your custom theme accordingly.

Additionally need to add Reablocks [default tokens](#9x-theme-tokens) to your project if you didn't do it before:
...
Removed `default`, `success`, `error` and `warning` color types. If Button component have used in project without default color property, please add `color="primary"`. Even if colors were removed from interface they still present in old theme and you can still use it
...
Along with breaking changes, we've introduced several new features: Typography (mentioned above) and Navigation component.
