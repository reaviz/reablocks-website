...
**Custom Theme**: When using `variant="custom"`, you're responsible for implementing light/dark mode support in your own CSS by targeting the `.theme-dark`, `.theme-light`, `[data-theme="dark"]`, or `[data-theme="light"]` selectors. The ThemeProvider will still observe theme changes, but your CSS must define the color values for each mode.
...
4. **Custom theme**: You define your own CSS variables and tokens that respond to theme class changes

This means you can use the same token in both light and dark modes, and it will automatically use the appropriate color value (for default and Unify themes).
