...
### Download the pre-built bundle

If you don't have Figma access or want a ready-made starting point, grab the full
theme bundle (5 CSS files + 46 TypeScript component themes) and drop it straight into
your project:

📦 **[Download UnifyTheme.zip](/assets/UnifyTheme.zip)**

The archive mirrors the [File layout](#file-layout) below — unzip it into
`src/assets/styles/` and `src/shared/utils/Theme/`, then jump to
[Wire it up at the app root](#wire-it-up-at-the-app-root).
...
The theme is split into two parts: **CSS token files** and a **TypeScript component-theme
module**:
...
You can keep this exact structure, or flatten it — the only requirement is that the CSS
...
The full file (~2500 lines) is generated alongside `root.css` by the Figma plugin and
produces utilities like:
