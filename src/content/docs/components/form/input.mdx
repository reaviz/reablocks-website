...
Use the `start` and `end` props (the deprecated `startAdornment` / `endAdornment`
props were removed in `10.0.0`):

```tsx
<Input start={<SearchIcon />} placeholder="Search..." />
<Input end={<span>USD</span>} type="number" />
```
